...
if (turn_around >= 0 && turn_around < 20) {
  ...
  if (looking_forward >= 0 && looking_forward < 20) {
    ...
    actions.push('Continue reinforcing positive behaviors and offer additional challenges.');
  ...
  if (using_phone >= 0 && using_phone < 20) {
    ...
    actions.push('Continue reinforcing positive behaviors and reward focus.');
    ...
    if (studentname && teacherUsername) {
      const fetchBehaviorData = async () => {
        try {
          ...
          setError("An error occurred while fetching behavior data.");
        } finally {
          setLoading(false);
        }
      };
    ...
    <div className="mt-40 min-w-3xl mx-auto p-6  text-white rounded-lg shadow-md space-y-6">
      ...
      {behaviorData && (
        <div className="bg-gray-800 p-4 rounded-lg shadow space-y-2">
          <h2 className="text-xl font-semibold mb-2">Behavior Data</h2>
